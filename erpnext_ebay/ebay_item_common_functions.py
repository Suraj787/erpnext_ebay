...
def get_ebay_category_id_of_item(item):
    vwrite("getting ebay category id")
    category_id_query = """select ebay_category_id from `tabItem Group` where name='%s'""" % (item.get("item_group"))
    vwrite(category_id_query)
    category_id = None
    for category_id_row in frappe.db.sql(category_id_query, as_dict=1):
        category_id = category_id_row.get("ebay_category_id")
    return category_id
...
def get_ebay_item_specifics(item):
    vwrite(item)
    vwrite("^^^^^^Item^^^^^^")
    return False
    item_specifics = []
    if (get_ebay_category_id_of_item(item)=='15032'):
        item_specifics = [
                    {
                        "Name": "Brand",
                        "Value": "Samsung"
    ...
    elif (get_ebay_category_id_of_item(item)=='172312'): # for TV
        item_specifics = [
                    {
                        "Name": "Brand",
                        "Value": "Samsung"
